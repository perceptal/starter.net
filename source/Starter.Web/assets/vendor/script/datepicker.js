DatePicker = (function($) {
	...
	function DatePicker(el, opts) {
		...
		$.extend(this, DatePicker.DEFAULT_OPTS, opts);
	...
	DatePicker.prototype = {
		build: function() {
			...
			this.wrapp = this.input.wrap('<div class="datepicker">');
			...
			this.yearNameSpan.dblclick(this.bindToObj(function() {
				...
				if ($('.year_name input', this.rootLayers).length == 0) {
					...
					$(".year_input", yearNav).keyup(this.bindToObj(function() {
						...
						} else if ($('input', this.yearNameSpan).val().length > 4) $('input', this.yearNameSpan).val($('input', this.yearNameSpan).val().substr(0, 4));
					}));
		...
		selectMonth: function(date) {
				...
				if (!this.currentMonth || !(this.currentMonth.getFullYear() == newMonth.getFullYear() && this.currentMonth.getMonth() == newMonth.getMonth())) {
					...
					for (var i = 0; i <= numDays; i++) {
						...
						if (this.isFirstDayOfWeek(currentDay)) {
							...
							if (this.select_week && this.isNewDateAllowed(firstDayOfWeek)) dayCells += "<tr date='" + this.dateToString(currentDay) + "' class='selectable_week'>";
							else dayCells += "<tr>";

							if (this.show_week == 1) dayCells += '<td class="week_num">' + this.getWeekNum(currentDay) + '</td>';
						}
			...
			} else this.show_error(this.error_out_of_range);
		},
		...
		selectDate: function(date) {
			...
			if (this.select_week == 1 && !this.isFirstDayOfWeek(date)) {
				date = new Date(date.getFullYear(), date.getMonth(), (date.getDate() - date.getDay() + this.start_of_week), 12);
			}
		...
		stringToDate: function(string) {
			...
			if (matches = string.match(this.reg)) {
				if (matches[3] == 0 && matches[2] == 0 && matches[1] == 0) return null;
				else return eval(this.date_decode);
			} else {
				return null;
			};
		},
		...
		moveMonthBy: function(amount) {
			if (amount < 0) var newMonth = new Date(this.currentMonth.getFullYear(), this.currentMonth.getMonth() + amount + 1, -1);
			else var newMonth = new Date(this.currentMonth.getFullYear(), this.currentMonth.getMonth() + amount, 1);
		...
		getMonthSelect: function() {
			...
			for (var i = 0; i < this.month_names.length; i++) {
				if (i == this.currentMonth.getMonth()) month_select += '<option value="' + (i) + '" selected="selected">' + this.month_names[i] + '</option>';
				else month_select += '<option value="' + (i) + '">' + this.month_names[i] + '</option>';
			}
			month_select += '</select>';
		...
		show_error: function(error) {
			...
			$('.error_msg', this.rootLayers).slideDown(400, function() {
				setTimeout("$('.error_msg', this.rootLayers).show();", 2000);
			});
		},
